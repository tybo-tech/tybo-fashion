...
class JobItemTransaction
{
  ...
  private function mutate($CompanyId, $JobId, $JobItemId, $requireItem, $itemMutation)
  {
    ...
    try {
      ...
      // Read the saved garment BEFORE commit so an add/update transaction
      // can never commit and then return garment: null.
      $garment = $resultJobItemId !== null
        ? $this->readGarment($resultJobItemId)
        : null;
      if ($resultJobItemId !== null && $garment === null) {
        throw new RuntimeException('Saved garment could not be read back.');
      }
  ...
  private function readGarment($JobItemId)
  {
    ...
    private function assertUnitPrice($value)
    {
        if ($value === null || !is_numeric($value) || (float) $value < 0) {
            throw new JobItemTransactionException('UnitPrice must be a non-negative number.', 400);
        }
    }

    private function assertQuantity($value)
    {
        if (
            $value === null
            || !is_numeric($value)
